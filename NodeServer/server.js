...
app.post('/subjects', (req, res) => {
    const body = req.body;
    appLibrary.getAllSubjects().then((result) => { console.log(result); res.send(result) });
});

app.post('/login', async (req, res) => {
    const userCredintial = req.body;
    appLibrary.login(userCredintial.userName, userCredintial.password).then((result) => { res.send(result) });
});
...
app.post('/postreview', async (req, res) => {
    ...
    var studentId = null;
    appLibrary.getStudentFromToken(reviewData.token).then((result) => { studentId = result.studentId });
    console.log(studentId);
    appLibrary.postReview(reviewData.userToken, reviewData.selectedSubject.id, reviewData.isCustom, reviewData.isAutoReview, reviewData.urgencySelection.value, reviewData.selectedQuestion.value, reviewData.selectedSubjectYear.value, reviewData.customQuestionText).then((result) => { res.send(result) });
});
...
app.post('/completedreviews', async (req, res) => {
    ...
    await appLibrary.getStudentFromToken(reviewData.userToken).then((result) => {
        ...
        console.log(studentId);
        appLibrary.getStudentCompletedFRQs(studentId).then((result) => { res.send(result) });
    });
});
...
app.post('/openreviews', async (req, res) => {
    ...
    await appLibrary.getStudentFromToken(reviewData.userToken).then((result) => {
        ...
        studentId = JSON.parse(JSON.stringify(result))[0].studentId;
        console.log(studentId);
        appLibrary.getStudentNotCompletedFRQs(studentId).then((result) => { res.send(result) });
    });
});
